...
- container: ANI demuxer now surfaces the `AF_ICON`-clear raw-BMP `iBitCount` as a 4-byte little-endian `extradata` block (alongside the `iWidth`/`iHeight` it already carried) — a headerless raw body needs the bit-depth as well as width/height to decode. The `AF_ICON`-set path leaves `extradata` empty (the per-frame ICO/CUR directory is authoritative there, matching the width/height sentinel handling)
- writer: emit **1/4/8-bpp indexed** and **24-bpp BGR** BMP sub-images, not just 32-bpp BGRA. New `WriteOptions::bmp_bit_depth` (`BmpBitDepth`, default `Bgra32`) selects the BMP-DIB depth: `Indexed8`/`Indexed4`/`Indexed1` quantise each BMP-bound sub-image to a palette built by exact-colour collection (erroring with the entry index when the image needs more than the depth's `2^bpp` colours, so a successful encode is colour-exact), `Rgb24` drops alpha to the 1-bpp AND mask, `Bgra32` keeps the existing path. The directory `wBitCount` now matches the body's `biBitCount` so a re-read passes the directory-vs-body cross-check. PNG-routed entries are unaffected. The framework-free encoders `encode_indexed_dib_body` / `encode_rgb24_dib_body` / `quantise_rgba_to_indexed` (+ `PaletteEntry`) build the doubled-`biHeight` + RGBQUAD palette + XOR rows + AND mask in-crate (the packing is intrinsic to the ICO sub-image; `oxideav-bmp`'s `encode_dib_plane` only appends the AND mask on its 32-bpp path) and are exported for `write_ico_raw`-driving callers. Spec `docs/image/ico/ico-cur-format.md` §"DIB form structure"
- writer: `WriteOptions::per_image_bit_depth` — when set, each BMP-bound sub-image is encoded at the depth its own `IconImage::bit_depth` names (via the new `BmpBitDepth::from_bits`, mapping `1/4/8/24/32`), so one `write_ico` call can emit a faithful **mixed-depth** multi-resolution icon (a legacy 1-bpp 16×16 beside a 32-bpp 256×256 — the shape a decoded real-world `.ico` round-trips into). An unencodable depth (`16`, or anything outside `1/4/8/24/32`) falls back to `bmp_bit_depth` rather than erroring
- write_ani: indexed / 24-bpp ANI frames now round-trip pixel-exact through `read_ani` (the encode chain routes `opts.ico` to `write_ico`, so an `AF_ICON` animation inherits the new low-bit-depth path) — 8-bpp and 1-bpp monochrome animation round-trip tests added, plus the previously decode-only 4-bpp case now covered on encode
- registry: add integration tests for the framework registration glue (previously untested). Registers everything into real `CodecRegistry` / `ContainerRegistry` instances and asserts the `"ico"` codec resolves a working decoder + encoder, the `"ico"` and `"ani"` demuxers (and `"ico"`-only muxer — ANI is demux-only) are present, the `ico` / `cur` / `ani` extensions route to the right container, `probe_input` routes ICO magic to `"ico"` and RIFF/ACON magic to `"ani"`, `open_demuxer("ico", …)` yields packets, and the `From<IcoError> for oxideav_core::Error` bridge maps both variants 1:1 with the message preserved
- codec: add direct tests for the framework `Decoder` / `Encoder` trait impls and the `make_decoder` / `make_encoder` factories (previously the codec layer had no tests at all). Covers an encode→packet→decode BMP round-trip (pixel-exact, since the < 64 px path is lossless BMP-DIB), the PNG-body path at the 64 px threshold, `make_encoder` defaulting `pixel_format` to RGBA while carrying width/height, `NeedMore`→`Eof` streaming semantics on both sides, and the missing-width / non-video-frame encoder error paths
- container: add tests for the framework `Muxer` (`IcoMuxer`), previously untested — a demux→re-mux→re-parse round-trip confirming `write_header` / `write_packet` / `write_trailer` lay out a directory the parser reads back to the same sub-images with payloads verbatim, plus the no-packets and packet-count-mismatch error paths
- container: the ICO/CUR framework `Demuxer` now delegates its directory walk to the hardened standalone `read_ico_raw` parser instead of carrying a thinner in-line copy that only bounds-checked payload extents. The demuxer now inherits the full parser CVE-surface hardening — overlapping sub-image payload ranges, non-zero `bReserved`, out-of-range `wPlanes` / `wBitCount`, CUR hotspot outside the body-derived sub-image, directory-vs-body dimension / bit-depth disagreement, and illegal `biSize` / `biPlanes` / `biCompression` — and reports the resolved-and-cross-validated sub-image dimensions on each stream rather than the directory's raw `u8` self-claim. One parser, one rule set, so the demuxer and the standalone API can never diverge on what a well-formed file is. `.ani` input is still refused cleanly
- tests: cover the indexed / low-bit-depth BMP-inside-ICO decode path through `read_ico` end-to-end — 1-bpp monochrome, 4-bpp, and 8-bpp palette sub-images with a hand-built `BITMAPINFOHEADER` + RGBQUAD palette + bottom-up XOR rows + 1-bpp AND mask (spec §"DIB form structure" / §"Decoder checklist" steps 5-7). Confirms palette lookup, bottom-up→top-down row flip, per-bit-depth stride packing, and AND-mask transparency (a set mask bit yields `alpha = 0`) all compose correctly. Previously every `read_ico` test exercised only 32-bpp (`IconImage::from_rgba` + `write_ico` emit 32-bpp), so the classic indexed-icon path had no integration coverage
- anih.cbSize: validate the field on read + write (spec §'anih': "validate `cbSize`"). The nine ANIHEADER fields occupy 36 bytes, so a self-reported `cbSize < 36` cannot describe the structure and is rejected up front (probe-vs-render guard); `cbSize > 36` stays tolerated (the "some encoders write a slightly different cbSize" caveat — the chunk length is the authoritative bound). `write_ani_raw` mirrors the reject so a value the reader bounces is never emitted
- read_ani: per-frame / per-step renderer accessors — `AniFrame::primary_image()` (the largest sub-image a renderer displays, via the `select_largest` heuristic), `AniFrame::hotspot()` (its cursor click point; `None` for `Ico` frames), and `AniAnimation::frame_at_step(i)` / `hotspot_at_step(i)` (resolve the `seq ` indirection so the displayed frame and its hotspot come back directly). An animated cursor's hotspot can change frame to frame, so a renderer driving the timeline (`step_at_jiffy` → `hotspot_at_step`) now reads the active click point per tick instead of digging through `frames[steps[i].frame_index].images[…].hotspot` by hand
- selection: add `select_best_fit_raw` / `select_largest_raw` / `select_by_dimensions_raw` — the directory-level counterparts of the decoded `select_*` family, running the same heuristics (and tie-break rules) over the undecoded `IconEntryRaw` rows from `read_ico_raw`. A caller can now pick the right sub-image from `bWidth`/`bHeight`/`wBitCount` *before* decoding any PNG/BMP body — the order Windows' `LookupIconIdFromDirectoryEx` resolves an icon in — instead of decoding every sub-image just to call the decoded selectors. Available with `default-features = false` (no `oxideav-core` needed). The decoded and raw families share a single generic core so they always agree on the same (width, height, bit-depth) facts
- container: ANI demuxer surfaces `anih` `iWidth`/`iHeight` on the stream's `CodecParameters` when set (the `0` = "take from frame" sentinel leaves them `None`), and carries `AF_ICON`-clear raw headerless-BMP frame bytes through verbatim — the only place that frame geometry lives, since raw bodies have no per-frame header
